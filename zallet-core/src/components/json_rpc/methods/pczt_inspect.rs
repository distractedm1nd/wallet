...
use super::pczt_common::{
    PROP_BACKEND_PROPOSAL_INFO, PROP_PRIVACY_POLICY, decode_pczt_base64, read_signing_hints,
};
...
/// Summarizes an Orchard-family bundle.
///
/// The Orchard and Ironwood bundles of a PCZT are the same type — Ironwood
/// notes are Orchard-shaped — so one summary serves both. Only the
/// signature count and the proof status are read from outside the bundle,
/// since both come from whole-PCZT queries.
fn orchard_info(
    ...
    OrchardInfo {
        ...
        outputs: bundle
            .actions()
            .iter()
            .map(|action| ShieldedOutputInfo {
                value_zat: *action.output().value(),
                user_address: action.output().user_address().clone(),
            })
            .collect(),
        value_balance_zat: value_balance(bundle.value_sum()),
        proof_complete,
    }
}
...
pub(crate) fn call(params: &Network, pczt_base64: &str) -> Response {
    ...
    let signing_hints = read_signing_hints(&pczt)
        .ok()
        .flatten()
        .map(|hints| SigningHintsInfo {
            seed_fingerprint: hints.seed_fp.to_string(),
            account_index: hints.account_idx.into(),
        });
    ...
    let sapling_outputs = pczt
        .sapling()
        .outputs()
        .iter()
        .map(|out| ShieldedOutputInfo {
            value_zat: *out.value(),
            user_address: out.user_address().clone(),
        })
        .collect();
    ...
    let orchard = orchard_info(
        pczt.orchard(),
        orchard_signed,
        !prover.requires_orchard_proof(),
    );
    let ironwood = orchard_info(
        pczt.ironwood(),
        ironwood_signed,
    ...
    Ok(InspectResult {
        ...
        transparent,
        sapling: SaplingInfo {
            spends: pczt.sapling().spends().len(),
