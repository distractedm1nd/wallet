...
pub(super) const PARAM_PRIVACY_POLICY_DESC: &str = "The most the transaction is permitted to reveal, using the same values as `z_sendmany`. \
     Defaults to \"FullPrivacy\".";
...
pub(crate) async fn call(
    ...
    // `create_pczt_from_proposal` supports single-step proposals only. Catching
    // it here names the offending address, instead of reporting a generic
    // "Failed to create PCZT" after the input selection has already run.
    for payment in request.payments().values() {
        let recipient = payment.recipient_address();
        if let Ok(Address::Tex(_)) =
            Address::try_from_zcash_address(wallet.params(), recipient.clone())
        {
            return Err(LegacyCode::InvalidParameter
                .with_message(fl!("err-pczt-tex-recipient", address = recipient.encode())));
        }
    }
...
pub(super) fn build_pczt(
    ...
    // `create_pczt_from_proposal` rejects multi-step proposals. The only one
    // Zallet can currently produce is the ZIP 320 (TEX) flow, which `call`
    // rejects up front; this covers any other route to a multi-step proposal
    // that the proposer might grow later.
    if proposal.steps().len() > 1 {
        return Err(LegacyCode::InvalidParameter.with_message(fl!("err-pczt-multi-step-proposal")));
    }
    ...
    let pczt = create_pczt_from_proposal::<_, _, Infallible, _, Infallible, _>(
        ...
        // padding must match it. `pczt_sign` relies on this too: the padding
        // dummies are signed here, at creation, and it skips them rather than
        // reporting them unsigned.
