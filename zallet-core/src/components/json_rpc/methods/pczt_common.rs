...
///
/// The round trip through `ChildNumber` is what forces the `bip32` dependency:
/// `TransparentKeyScope` wraps a `u32` but exposes no accessor for it, and
/// `From<TransparentKeyScope> for ChildNumber` is the only public way to read
/// it back out. An upstream `to_u32` (the inverse of the existing
/// [`TransparentKeyScope::custom`]) would let this drop the dependency.
...
/// The account signing hints recorded in a PCZT by `pczt_create`.
pub(super) struct SigningHints {
    pub(super) seed_fp: SeedFingerprint,
    pub(super) account_idx: AccountId,
}
...
/// the wallet simply has no way to locate its keys.
///
/// Hints that are present but malformed are rejected: they indicate a corrupted
/// or tampered PCZT, not a foreign one. A caller that would rather describe a
/// questionable PCZT than reject it can treat that as “no hints” with
/// `.ok().flatten()`.
///
/// This is the only place that knows the hints' byte layout, so the reader
/// cannot drift from [`PROP_SEED_FINGERPRINT`] and [`PROP_ACCOUNT_INDEX`] as
/// `pczt_create` writes them.
pub(super) fn read_signing_hints(pczt: &Pczt) -> RpcResult<Option<SigningHints>> {
...
/// A process-lifetime cache holding one value per Orchard-family circuit
/// version.
///
/// The versions are a closed set, so a fixed array gives each its own
/// [`OnceLock`] — no map, and no locking on the read path once built. Keeping
/// the version-to-slot mapping in one place means adding a circuit version
/// touches this type rather than every cache keyed by one.
struct PerCircuitVersion<T>([OnceLock<T>; 3]);

impl<T> PerCircuitVersion<T> {
    const fn new() -> Self {
        Self([OnceLock::new(), OnceLock::new(), OnceLock::new()])
    }

    /// Returns the cached value for `version`, building it on first use.
    fn get_or_init(
        &self,
        version: OrchardCircuitVersion,
        build: impl FnOnce(OrchardCircuitVersion) -> T,
    ) -> &T {
        let slot = match version {
            OrchardCircuitVersion::InsecurePreNu6_2 => &self.0[0],
            OrchardCircuitVersion::FixedPostNu6_2 => &self.0[1],
            OrchardCircuitVersion::PostNu6_3 => &self.0[2],
        };
        slot.get_or_init(|| build(version))
    }
}
...
pub(super) fn orchard_proving_key(version: OrchardCircuitVersion) -> &'static ProvingKey {
    static KEYS: PerCircuitVersion<ProvingKey> = PerCircuitVersion::new();
    KEYS.get_or_init(version, ProvingKey::build)
}
...
pub(super) fn orchard_verifying_key(version: OrchardCircuitVersion) -> &'static VerifyingKey {
    static KEYS: PerCircuitVersion<VerifyingKey> = PerCircuitVersion::new();
    KEYS.get_or_init(version, VerifyingKey::build)
}
